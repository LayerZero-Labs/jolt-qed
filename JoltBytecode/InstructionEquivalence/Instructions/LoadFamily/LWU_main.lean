...
def lwuProgramEqSailStatement (imm : BitVec 12) (rs1 rd : regidx)
    (js : SailJoltState)
...
-- same as LW. Only the final extraction is unsigned: VirtualPext produces the
-- selected 32-bit word zero-extended to 64 bits.

/-- Unsigned PEXT with the generated word-window mask extracts the selected
word and zero-extends it to 64 bits. -/
theorem pext_word_window
  ...
  exact window_mask_w_pext
    (d := dval)
    (base := base)
    (imm := imm)
    (halign := hAlign)

/-- The final instruction of the non-`x0` LWU expansion writes the unsigned
parallel-extract result to the architectural destination register. -/
theorem lwu_virtual_pext_rd_run
...
/-- The final instruction of the `x0` LWU expansion writes the unsigned
parallel-extract result to virtual register 40. -/
theorem lwu_virtual_pext_v40_run
...
/-- The generated `LWU` expansion writes only instruction-local scratch
vregs. -/
...
/-- After either generated `LWU` destination branch retires, `systemProject`
agrees with the embedded Sail state. -/
theorem lwuProgramAuto_systemProject_eq_sail
    (imm : BitVec 12) (rs1 rd : regidx)
  ...
  have hprojected : Projection.ProjectedVRegsPreserved js js' :=
    lwuProgramAuto_preserves_projected_vregs
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (hrun := hrun)
...
/-- The aligned non-`x0` generated LWU expansion runs to its explicit final
state. The first four instruction facts are reused directly from LW. -/
theorem lwuProgramAuto_nonzero_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
  ...
  simp only [JoltISA.lwuProgramAuto, if_neg hx0]
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := js)
    (js' := js)
    (h := LW_main.lw_virtual_assert_word_alignment_run
      (imm := imm)
      (rs1 := rs1)
      (js := js)
      (h := h)
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := jsLoad)
    (js' := jsMask)
    (h := LW_main.lw_virtual_window_mask_w_run
      (dwordVReg := Vreg 41)
      (maskVReg := Vreg 40)
      (hWritable := hv40)
      (imm := imm)
...
/-- The aligned path through the non-`x0` LWU branch. -/
theorem lwuProgram_eq_sail_nonzero_aligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0)
  ...
  have hjolt :
      (JoltISA.execProgram (JoltISA.lwuProgramAuto rd rs1 imm)).run js =
        .ok RETIRE_SUCCESS jsPext := by
    exact lwuProgramAuto_nonzero_run
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
  ...
  have hProject : System.systemProject jsPext = jsPext.sail :=
    lwuProgramAuto_systemProject_eq_sail
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
  ...
  rw [hjolt]
  rw [LW_SailSide.execute_LWU_reduces
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
    (h := h)
    (hAlign := hAlign)]
  rw [LW_SailSide.stateAfterWrite_writeValueUnsigned_eq_dword_word
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
...
/-- A misaligned generated LWU expansion stops at its first instruction with
the load-address-alignment exception, independently of its destination. -/
theorem lwuProgramAuto_misaligned_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) ≠ 0) :
...
/-- The misaligned LWU path is common to both destination branches. -/
theorem lwuProgram_eq_sail_misaligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) ≠ 0) :
    lwuProgramEqSailStatement imm rs1 rd js h := by
  unfold lwuProgramEqSailStatement
  have hjolt := lwuProgramAuto_misaligned_run
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
  ...
  have hsail := LW_SailSide.execute_LWU_misaligned
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
...
/-- The complete non-`x0` branch of the main LWU program theorem. -/
theorem lwuProgram_eq_sail_nonzero
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hx0 : JoltISA.isX0 rd ≠ true) :
    lwuProgramEqSailStatement imm rs1 rd js h := by
  by_cases hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0
  · exact lwuProgram_eq_sail_nonzero_aligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
      ...
      (hAlign := hAlign)
      (hx0 := hx0)
  · exact lwuProgram_eq_sail_misaligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
...
/-- The aligned `x0` generated LWU expansion runs to its explicit final state.
The first four instruction facts are reused directly from LW. -/
theorem lwuProgramAuto_x0_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0)
    ...
    (JoltISA.execProgram (JoltISA.lwuProgramAuto rd rs1 imm)).run js =
      .ok RETIRE_SUCCESS jsPext := by
  simp only [JoltISA.lwuProgramAuto, hx0, if_true]
  have hv40 : WritableVReg (Vreg 40) := by
    simp only [WritableVReg, Vreg, LW_main.Vreg, BitVec.toNat_ofNat]
    norm_num
  have hv41 : WritableVReg (Vreg 41) := by
...
/-- The aligned path through the `x0` LWU branch. -/
theorem lwuProgram_eq_sail_x0_aligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0)
...
/-- The complete `x0` branch of the main LWU program theorem. -/
theorem lwuProgram_eq_sail_x0
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hx0 : JoltISA.isX0 rd = true) :
    lwuProgramEqSailStatement imm rs1 rd js h := by
  by_cases hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0
  · exact lwuProgram_eq_sail_x0_aligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
...
/-- **Main program theorem for LWU.** The structured Jolt-ISA expansion
matches Sail after materializing Jolt's persistent CSR virtual registers. -/
theorem lwuProgram_eq_sail
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js) :
    lwuProgramEqSailStatement imm rs1 rd js h := by
  by_cases hx0 : JoltISA.isX0 rd = true
  · exact lwuProgram_eq_sail_x0
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
      (h := h)
      (hx0 := hx0)
  · exact lwuProgram_eq_sail_nonzero
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
