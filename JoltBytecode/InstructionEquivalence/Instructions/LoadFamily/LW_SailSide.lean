...
namespace LW_SailSide

/-- A failed leading word load-alignment assertion stops the structured
...
theorem assertWordBlockMisaligned (tail : JoltISA.Program)
    ...
    (JoltISA.execProgram
      (.instr (JoltISA.Encoded.VirtualAssertWordAlignment rs1 imm (ExceptionType.E_Load_Addr_Align ()))
        tail)).run js =
...
/-- The word selected by an aligned `LW` fits inside its enclosing dword. -/
theorem word_offset_fits (base : BitVec 64) (imm : BitVec 12)
    (hAlign : load_effective_address base imm &&& (3 : BitVec 64) = 0) :
    (load_effective_address base imm &&& (7 : BitVec 64)).toNat + 4 ≤ 8 := by
  have hlt := addr_and_seven_word_lt_five
    (load_effective_address base imm) hAlign
  omega

/-- Splitting an effective address into its aligned dword base and word offset
recovers the original address. -/
theorem aligned_base_add_word_offset (base : BitVec 64) (imm : BitVec 12) :
...
/-- The aligned word subaccess cannot wrap while locating its start inside
the enclosing dword. -/
theorem word_subaccess_no_overflow
  ...
  have hfit := word_offset_fits h.rs1_val imm hAlign
  omega

/-- The enclosing Jolt dword window contains Sail's word-sized `LW` access. -/
theorem word_mem_present
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
    MemBytesPresentAt js.sail (load_effective_address h.rs1_val imm) 4 := by
  simpa only [aligned_base_add_word_offset] using
    memBytesPresentAt_subaccess
      ...
      (word_offset_fits h.rs1_val imm hAlign)
      (word_subaccess_no_overflow imm rs1 js h hAlign)

/-- The enclosing Jolt load-PMP window authorizes Sail's word-sized access. -/
theorem word_load_pmp_ok
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
    Assumptions.LoadPmpOk
      (load_effective_address h.rs1_val imm) 4 js.sail := by
  simpa only [aligned_base_add_word_offset] using
...
/-- The enclosing Jolt non-MMIO window contains Sail's word-sized access. -/
theorem word_not_readable_mmio
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
...
/-- An aligned effective word address has room for all four bytes. -/
theorem word_no_overflow
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
    (load_effective_address h.rs1_val imm).toNat + 3 < 2 ^ 64 :=
  aligned_word_addr_no_ovf
    (load_effective_address h.rs1_val imm) hAlign

/-- Sail's alignment evidence for an aligned word load. -/
theorem aligned_access
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
    AlignedAccess (load_effective_address h.rs1_val imm) 4 := by
  refine
    { misalign := ?_
      split := ?_ }
  · exact access_misaligned_4_aligned_false
      (load_effective_address h.rs1_val imm) hAlign
  · exact split_misaligned_aligned_4
      (load_effective_address h.rs1_val imm) hAlign

/-- The architectural value written by Sail's aligned `LW`. -/
abbrev writeValue
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
    ...
    BitVec 64 :=
  sign_extend (m := 64)
    (loaded_word_at js.sail
      (load_effective_address h.rs1_val imm)
      (word_mem_present imm rs1 js h hAlign)
      (word_no_overflow imm rs1 js h hAlign))

/-- The architectural value written by Sail's aligned `LWU`. -/
...
/-- Writing Sail's directly loaded word is the same as writing the selected
word of the enclosing loaded dword. -/
theorem stateAfterWrite_writeValue_eq_dword_word
    ...
    stateAfterWrite js.sail rd (writeValue imm rs1 js h hAlign) =
      stateAfterWrite js.sail rd
        (sign_extend (m := 64) (word_of_dword dval wordOffset)) := by
  simp only
  unfold writeValue
  rw [loaded_word_in_dword
    (s := js.sail)
    ...
    (hbytes_addr := word_mem_present imm rs1 js h hAlign)
    (h_no_ovf_addr := word_no_overflow imm rs1 js h hAlign)]

/-- Writing Sail's directly loaded unsigned word is the same as writing the
selected word of the enclosing loaded dword. -/
theorem stateAfterWrite_writeValueUnsigned_eq_dword_word
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
    ...
    stateAfterWrite js.sail rd (writeValueUnsigned imm rs1 js h hAlign) =
      stateAfterWrite js.sail rd
        (zero_extend (m := 64) (word_of_dword dval wordOffset)) := by
  simp only
...
/-- Sail-side aligned `execute_LOAD imm rs1 rd false 4` reduces to a register
write of the sign-extended loaded word. -/
theorem execute_LW_reduces
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
  ...
  rw [vmem_read_word_reduces
    ...
    (value := loaded_word_at js.sail
      (load_effective_address h.rs1_val imm)
      (word_mem_present imm rs1 js h hAlign)
      (word_no_overflow imm rs1 js h hAlign))
    (h_mem := mem_read_4_eq_loaded_word
      (addr := load_effective_address h.rs1_val imm)
      (s := js.sail)
      (hpriv := h.cur_privilege)
      (hmprv := h.mstatus_mprv)
      (h_no_ovf := word_no_overflow imm rs1 js h hAlign)
      (hbytes := word_mem_present imm rs1 js h hAlign)
      (hpmp := word_load_pmp_ok imm rs1 js h hAlign)
      (hmmio := word_not_readable_mmio imm rs1 js h hAlign))]
...
/-- Sail-side aligned `execute_LOAD imm rs1 rd true 4` reduces to a register
write of the zero-extended loaded word. -/
theorem execute_LWU_reduces
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) = 0) :
...
/-- Sail-side misaligned `LW` stops with a load-address-alignment exception. -/
theorem execute_LW_misaligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) ≠ 0) :
  ...
  have hmis :
    ...
    simpa only [ea] using access_misaligned_4_unaligned_true ea hMisaligned
...
/-- Sail-side misaligned `LWU` stops with the same load-address-alignment
exception as signed `LW`. -/
theorem execute_LWU_misaligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (3 : BitVec 64) ≠ 0) :
