...
import JoltBytecode.InstructionEquivalence.Instructions.LoadFamily.LH_SailSide
...
abbrev Vreg x := LW_main.Vreg x

/-- **Main program theorem for LH.** The structured Jolt-ISA expansion matches
Sail after materializing Jolt's persistent CSR virtual registers. -/
def lhProgramEqSailStatement (imm : BitVec 12) (rs1 rd : regidx)
    (js : SailJoltState)
...
-- LH follows the same proof structure as LW. The differences are the
-- halfword-alignment assertion, VirtualWindowMaskH, and a 16-bit signed PEXT
-- result corresponding to loaded_halfword_at.

/-- Signed PEXT with the generated halfword-window mask extracts the selected
halfword and sign-extends it to 64 bits. -/
theorem pext_signed_halfword_window
  ...
  exact window_mask_h_pext_signed
    (d := dval)
    (base := base)
    (imm := imm)
    (halign := hAlign)

/-- The first LH instruction confirms that the effective address is halfword
aligned and leaves the state unchanged. -/
theorem lh_virtual_assert_halfword_alignment_run
  ...
  simpa only [load_effective_address] using
    JoltISA.virtual_assert_halfword_alignment_run_aligned
      (base := rs1)
      (imm := imm)
...
/-- The fourth LH instruction writes the halfword-window mask to its chosen
scratch virtual register. -/
theorem lh_virtual_window_mask_h_run
...
/-- The final instruction of the non-`x0` LH expansion writes the signed
parallel-extract result to the architectural destination register. -/
theorem lh_virtual_pext_signed_rd_run
  ...
  simpa only [stateAfterVRegWrite, Vreg, LW_main.Vreg, BitVec.reduceEq,
    ↓reduceIte] using
...
/-- The final instruction of the `x0` LH expansion writes the signed
parallel-extract result to virtual register 40. -/
theorem lh_virtual_pext_signed_v40_run
    (imm : BitVec 12) (rs1 : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js) :
    let facts := h.dwordWindowFacts
    let daddr := compute_aligned_dword_base_address h.rs1_val imm
  ...
  simp only [WritableVReg, Vreg, LW_main.Vreg, BitVec.toNat_ofNat]
  norm_num

/-- The generated `LH` expansion writes only instruction-local scratch
vregs. -/
...
/-- After either generated `LH` destination branch retires, `systemProject`
agrees with the embedded Sail state. -/
theorem lhProgramAuto_systemProject_eq_sail
    (imm : BitVec 12) (rs1 rd : regidx)
  ...
  have hprojected : Projection.ProjectedVRegsPreserved js js' :=
    lhProgramAuto_preserves_projected_vregs
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (hrun := hrun)
...
/-- The aligned non-`x0` generated LH expansion runs through its five
instructions to the explicit final state. Align-address and LD are reused from
LW because those instructions are width-independent. -/
theorem lhProgramAuto_nonzero_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0)
  ...
  simp only [JoltISA.lhProgramAuto, if_neg hx0]
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := js)
    (js' := js)
    (h := lh_virtual_assert_halfword_alignment_run
      (imm := imm)
      (rs1 := rs1)
      (js := js)
      (h := h)
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := jsLoad)
    (js' := jsMask)
    (h := lh_virtual_window_mask_h_run
      (dwordVReg := Vreg 41)
      (maskVReg := Vreg 40)
      (hWritable := hv40)
      (imm := imm)
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := jsMask)
    (js' := jsPext)
    (h := lh_virtual_pext_signed_rd_run
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
...
/-- The aligned path through the non-`x0` LH branch. -/
theorem lhProgram_eq_sail_nonzero_aligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0)
  ...
  have hjolt :
      (JoltISA.execProgram (JoltISA.lhProgramAuto rd rs1 imm)).run js =
        .ok RETIRE_SUCCESS jsPext := by
    exact lhProgramAuto_nonzero_run
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
  ...
  have hProject : System.systemProject jsPext = jsPext.sail :=
    lhProgramAuto_systemProject_eq_sail
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
  ...
  rw [hjolt]
  rw [LH_SailSide.execute_LH_reduces
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
    (h := h)
    (hAlign := hAlign)]
  rw [LH_SailSide.stateAfterWrite_writeValue_eq_dword_halfword
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
  ...
  -- NOTE: Math lemma
  rw [← pext_signed_halfword_window
    (dval := dval)
    (base := h.rs1_val)
    (imm := imm)
    (hAlign := hAlign)]
...
/-- A misaligned generated LH expansion stops at its first instruction with
the load-address-alignment exception, independently of its destination. -/
theorem lhProgramAuto_misaligned_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) ≠ 0) :
...
/-- The misaligned LH path is common to both destination branches. -/
theorem lhProgram_eq_sail_misaligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hMisaligned :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) ≠ 0) :
    lhProgramEqSailStatement imm rs1 rd js h := by
  unfold lhProgramEqSailStatement
  have hjolt := lhProgramAuto_misaligned_run
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
  ...
  have hsail := LH_SailSide.execute_LH_misaligned
    (imm := imm)
    (rs1 := rs1)
    (rd := rd)
    (js := js)
...
/-- The complete non-`x0` branch of the main LH program theorem. -/
theorem lhProgram_eq_sail_nonzero
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hx0 : JoltISA.isX0 rd ≠ true) :
    lhProgramEqSailStatement imm rs1 rd js h := by
  by_cases hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0
  · exact lhProgram_eq_sail_nonzero_aligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
      ...
      (hAlign := hAlign)
      (hx0 := hx0)
  · exact lhProgram_eq_sail_misaligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
...
/-- The aligned `x0` generated LH expansion runs through its five instructions
to the explicit final state. -/
theorem lhProgramAuto_x0_run
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0)
    ...
    (JoltISA.execProgram (JoltISA.lhProgramAuto rd rs1 imm)).run js =
      .ok RETIRE_SUCCESS jsPext := by
  simp only [JoltISA.lhProgramAuto, hx0, if_true]
  have hv40 : WritableVReg (Vreg 40) := by
    simp only [WritableVReg, Vreg, LW_main.Vreg, BitVec.toNat_ofNat]
    norm_num
  have hv41 : WritableVReg (Vreg 41) := by
  ...
  rw [JoltISA.execProgram_instr_run_retire
    ...
    (js := jsMask)
    (js' := jsPext)
    (h := lh_virtual_pext_signed_v40_run
      (imm := imm)
      (rs1 := rs1)
      (js := js)
      (h := h))]
...
/-- The aligned path through the `x0` LH branch. -/
theorem lhProgram_eq_sail_x0_aligned
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0)
...
/-- The complete `x0` branch of the main LH program theorem. -/
theorem lhProgram_eq_sail_x0
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js)
    (hx0 : JoltISA.isX0 rd = true) :
    lhProgramEqSailStatement imm rs1 rd js h := by
  by_cases hAlign :
      load_effective_address h.rs1_val imm &&& (1 : BitVec 64) = 0
  · exact lhProgram_eq_sail_x0_aligned
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
...
/-- **Main program theorem for LH.** The structured Jolt-ISA expansion matches
Sail after materializing Jolt's persistent CSR virtual registers. -/
theorem lhProgram_eq_sail
    (imm : BitVec 12) (rs1 rd : regidx) (js : SailJoltState)
    (h : LoadProgramEqSailAssumptions imm rs1 js) :
    lhProgramEqSailStatement imm rs1 rd js h := by
  by_cases hx0 : JoltISA.isX0 rd = true
  · exact lhProgram_eq_sail_x0
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
      (h := h)
      (hx0 := hx0)
  · exact lhProgram_eq_sail_nonzero
      (imm := imm)
      (rs1 := rs1)
      (rd := rd)
      (js := js)
