...
private theorem amoandwProgram_core_eq_sail
  ...
  · let base := amoWordBase addr
    ...
    have hatomic_pmp_word : Assumptions.AtomicPmpOk amoop.AMOAND addr 4 js.sail := by
      have hsub :
          Assumptions.AtomicPmpOk amoop.AMOAND
    ...
    rcases
        amo_word_binop_program_concrete_aligned
          ...
          (amo_word_and_result_extract_eq addr rs2Val dword oldWord hold)
          (amo_word_and_middle_after_pre_for rd rs2 js rs2Val dword
    ...
    have hsail :=
      execute_AMO_word_non_cas_aligned
        amoop.AMOAND rs2 rs1 rd js h.cur_privilege h.mstatus_mprv
        addr rs2Val
        ((Sail.BitVec.extractLsb rs2Val 31 0 : BitVec 32) &&& oldWord)
  ...
  · have hjolt :
      ...
      exact
        amo_word_pre64_with_scratch_misaligned_run rs1 (JoltISA.amoOldVRegFor rd)
          ...
          (.instr (.AND (.vreg (JoltISA.amoNewVRegFor rd))
    ...
    have hsail :=
      execute_AMO_word_misaligned
        amoop.AMOAND rs2 rs1 rd js addr rs2Val
