...
theorem amoorwProgram_doesNotWriteProtectedVRegs
    (rs2 rs1 rd : regidx) :
    JoltISA.ProgramWritesNoProtectedVReg
      (JoltISA.amoorwProgramAuto rd rs1 rs2) := by
  unfold JoltISA.amoorwProgramAuto
  split <;>
...
private theorem amoorwProgramCore_doesNotWriteProtectedVRegs
    (rs2 rs1 rd : regidx) :
    JoltISA.ProgramWritesNoProtectedVReg
...
private theorem amoorwProgram_core_eq_sail
  ...
  · let base := amoWordBase addr
    ...
    have hatomic_pmp_word : Assumptions.AtomicPmpOk amoop.AMOOR addr 4 js.sail := by
      have hsub :
          Assumptions.AtomicPmpOk amoop.AMOOR
    ...
    have hprojected : Projection.ProjectedVRegsPreserved js jsf :=
      Projection.execProgram_preserves_projected_vregs_of_no_protected_writes
        (amoorwProgramCore_doesNotWriteProtectedVRegs rs2 rs1 rd) hjolt
  ...
  · have hjolt :
    ...
    have hsail :=
      execute_AMO_word_misaligned
        amoop.AMOOR rs2 rs1 rd js addr rs2Val
...
def amoorwProgramEqSailStatement
  ...
  System.systemProjectResult
      ((JoltISA.execProgram
        (JoltISA.amoorwProgramAuto rd rs1 rs2)).run js) =
...
theorem amoorwProgram_eq_sail
  ...
  have hcore := amoorwProgram_core_eq_sail rs2 rs1 rd js h
  cases hrd : JoltISA.isX0 rd <;>
    simpa only [JoltISA.amoorwProgramAuto,
      JoltISA.amoWordBinopProgram,
      JoltISA.amoPre64ProgramWithScratch,
      JoltISA.amoPost64ProgramWithScratch,
