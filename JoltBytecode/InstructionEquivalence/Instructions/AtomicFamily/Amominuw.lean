...
theorem amominuwProgram_doesNotWriteProtectedVRegs
    (rs2 rs1 rd : regidx) :
    JoltISA.ProgramWritesNoProtectedVReg
      (JoltISA.amominuwProgramAuto rd rs1 rs2) := by
  unfold JoltISA.amominuwProgramAuto
  split <;>
...
private theorem amominuwProgramCore_doesNotWriteProtectedVRegs
    (rs2 rs1 rd : regidx) :
    JoltISA.ProgramWritesNoProtectedVReg
...
private theorem amominuwProgram_core_eq_sail
  ...
  · let base := amoWordBase addr
    ...
    have hprojected : Projection.ProjectedVRegsPreserved js jsf :=
      Projection.execProgram_preserves_projected_vregs_of_no_protected_writes
        (amominuwProgramCore_doesNotWriteProtectedVRegs rs2 rs1 rd) hjolt
...
def amominuwProgramEqSailStatement
  ...
  System.systemProjectResult
      ((JoltISA.execProgram
        (JoltISA.amominuwProgramAuto rd rs1 rs2)).run js) =
...
theorem amominuwProgram_eq_sail
  ...
  have hcore := amominuwProgram_core_eq_sail rs2 rs1 rd js h
  cases hrd : JoltISA.isX0 rd <;>
    simpa only [JoltISA.amominuwProgramAuto,
      JoltISA.amoWordSelectRustProgram,
      JoltISA.amoPre64ProgramWithScratch,
      JoltISA.amoPost64ProgramWithScratch,
