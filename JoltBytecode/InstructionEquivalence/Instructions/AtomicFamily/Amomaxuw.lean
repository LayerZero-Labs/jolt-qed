...
private theorem amomaxuwProgram_core_eq_sail
  ...
  · let base := amoWordBase addr
    ...
    have hatomic_pmp_word : Assumptions.AtomicPmpOk amoop.AMOMAXU addr 4 js.sail := by
      have hsub :
          Assumptions.AtomicPmpOk amoop.AMOMAXU
    ...
    rcases
        amo_word_rust_select_program_concrete_aligned
          ...
          (amo_word_rust_select_maxu_middle_after_pre_for rd rs2 js addr rs2Val dword
            h.rs2_read) with
      ⟨jsf, hjolt, hjolt_sail⟩
    have hsail :=
      execute_AMO_word_non_cas_aligned
        amoop.AMOMAXU rs2 rs1 rd js h.cur_privilege h.mstatus_mprv
        addr rs2Val
        (if (zopz0zK_u
            (Sail.BitVec.extractLsb rs2Val 31 0 : BitVec 32)
            oldWord : Bool) then
