...
theorem amomaxwProgram_doesNotWriteProtectedVRegs
    ...
    JoltISA.ProgramWritesNoProtectedVReg
      (JoltISA.amomaxwProgramAuto rd rs1 rs2) := by
  unfold JoltISA.amomaxwProgramAuto
  split <;>
...
private theorem amomaxwProgramCore_doesNotWriteProtectedVRegs
    (rs2 rs1 rd : regidx) :
    JoltISA.ProgramWritesNoProtectedVReg
...
private theorem amomaxwProgram_core_eq_sail
  ...
  · let base := amoWordBase addr
    ...
    have hatomic_pmp_word : Assumptions.AtomicPmpOk amoop.AMOMAX addr 4 js.sail := by
      have hsub :
          Assumptions.AtomicPmpOk amoop.AMOMAX
    ...
    rcases
        amo_word_rust_select_program_concrete_aligned
          ...
          (amo_word_max_result_extract_eq rs2Val (amoWordShiftedOld addr dword)
            oldWord hold)
          (by simpa [dword] using hold)
          (amo_word_rust_select_max_middle_after_pre_for rd rs2 js addr rs2Val dword
            h.rs2_read) with
      ⟨jsf, hjolt, hjolt_sail⟩
    have hsail :=
      execute_AMO_word_non_cas_aligned
        amoop.AMOMAX rs2 rs1 rd js h.cur_privilege h.mstatus_mprv
        addr rs2Val
        (if (zopz0zK_s
            (Sail.BitVec.extractLsb rs2Val 31 0 : BitVec 32)
            oldWord : Bool) then
    ...
    have hprojected : Projection.ProjectedVRegsPreserved js jsf :=
      Projection.execProgram_preserves_projected_vregs_of_no_protected_writes
        (amomaxwProgramCore_doesNotWriteProtectedVRegs rs2 rs1 rd) hjolt
...
def amomaxwProgramEqSailStatement
  ...
  System.systemProjectResult
      ((JoltISA.execProgram
        (JoltISA.amomaxwProgramAuto rd rs1 rs2)).run js) =
...
theorem amomaxwProgram_eq_sail
  ...
  have hcore := amomaxwProgram_core_eq_sail rs2 rs1 rd js h
  cases hrd : JoltISA.isX0 rd <;>
    simpa only [JoltISA.amomaxwProgramAuto,
      JoltISA.amoWordSelectRustProgram,
      JoltISA.amoPre64ProgramWithScratch,
      JoltISA.amoPost64ProgramWithScratch,
